// migrations/deploy/deploy_analytics.rs
...
fn main() {
    let root = init_root();
    let analytics_contract = deploy_contract::<AnalyticsContract>("analytics", &root, "../res/analytics_contract.wasm");

    // Edge case handling and settings (e.g., initial configurations, admin setting)
    root.call(
        analytics_contract.user_account.account_id(), 
        "initialize", 
