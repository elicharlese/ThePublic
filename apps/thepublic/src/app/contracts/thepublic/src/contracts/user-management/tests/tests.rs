#[cfg(test)]
mod tests {
    use super::*;
    use near_sdk::MockedBlockchain;
    use near_sdk::{testing_env, VMContext};

    fn get_context(is_view: bool) -> VMContext {
        VMContext {
            current_account_id: "alice.testnet".to_string(),
            signer_account_id: "bob.testnet".to_string(),
            signer_account_pk: vec![0, 1, 2],
            predecessor_account_id: "bob.testnet".to_string(),
            input: vec![],
            block_index: 0,
            block_timestamp: 0,
            account_balance: 0,
            account_locked_balance: 0,
            storage_usage: 0,
            attached_deposit: 0,
            prepaid_gas: 10u64.pow(18),
            random_seed: vec![0, 1, 2],
            is_view,
            output_data_receivers: vec![],
            epoch_height: 0,
        }
    }

    #[test]
    fn test_register_user() {
        let context = get_context(false);
        testing_env!(context);
        let mut contract = UserManagementContract::new();
        assert!(contract.register_user("alice".to_string(), "bio".to_string(), "alice@test.com".to_string()));
        let profile = contract.get_user_profile("alice.testnet".to_string()).unwrap();
        assert_eq!(profile.username, "alice");
        assert_eq!(profile.bio, "bio");
        assert_eq!(profile.email, "alice@test.com");
    }

    #[test]
    fn test_duplicate_user_registration() {
        let context = get_context(false);
        testing_env!(context);
        let mut contract = UserManagementContract::new();
        assert!(contract.register_user("alice".to_string(), "bio".to_string(), "alice@test.com".to_string()));
        assert!(!contract.register_user("alice".to_string(), "bio".to_string(), "alice@test.com".to_string()));
    }

    #[test]
    fn test_authenticate_user() {
        let context = get_context(false);
        testing_env!(context);
        let mut contract = UserManagementContract::new();
        contract.register_user("alice".to_string(), "bio".to_string(), "alice@test.com".to_string());
        assert!(contract.authenticate_user());
        let profile = contract.get_user_profile("alice.testnet".to_string()).unwrap();
        assert!(profile.authenticated);
    }

    #[test]
    fn test_update_user_profile() {
        let context = get_context(false);
        testing_env!(context);
        let mut contract = UserManagementContract::new();
        contract.register_user("alice".to_string(), "bio".to_string(), "alice@test.com".to_string());
        assert!(contract.update_user_profile(Some("new_alice".to_string()), Some("new_bio".to_string()), None));
        let profile = contract.get_user_profile("alice.testnet".to_string()).unwrap();
        assert_eq!(profile.username, "new_alice");
        assert_eq!(profile.bio, "new_bio");
        assert_eq!(profile.email, "alice@test.com");
    }
}